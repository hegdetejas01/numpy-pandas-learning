...
print(genres.min())
print(genres.mean(numeric_only=True)) # apply only on numeric data
...
print(movies.groupby('genre').size()) # sorted based on index
print(movies['genre'].value_counts()) # sorted based on values
...
print(genres.last())
# nth()
print(genres.nth(3)) # picks the 4th movies from each group (if present)



### getgroup()
# get all items of a particular group
print(genres.get_group('Fantasy')) # all movies of Fantasy group
...
print(genres.sample()) # one random movie from all genres
print(genres.sample(2, replace=True)) # gives 2 random movies from all genres
...
##### performimg 'min','max','mean','sum','median','std' on all columns
# genres.agg(['min','max','mean'], numeric_only=True)  - this code breaks
print(genres[['runtime','imdb_rating','no_of_votes','metascore','gross']].agg(['min','max','mean','sum','median','std']))
...
print(duo.size())
print(duo.get_group(('Abhishek Chaubey','Shahid Kapoor')))

# which actor - director duo has earned more
print(duo['gross'].sum().sort_values(ascending=False).head(1).reset_index())

# find the best (in terms of metascore(avg)) actor -> genre combo
actGen = movies.groupby(['star1','genre'])
print(actGen['metascore'].mean().reset_index().sort_values('metascore', ascending=False).head(1))

# agg on multiple groupby
print(duo[['gross','imdb_rating','runtime','metascore','no_of_votes']].agg(['min','max','mean']))
...
print(ipl[ipl['batsman_runs'] == 6].groupby('batsman')['batsman'].count().sort_values(ascending=False).head(1).index[0])

## find batsman with most number of 4's and 6's in last 5 overs
filter = ipl[(ipl['over'] > 15) & ((ipl['batsman_runs'] == 4) | (ipl['batsman_runs'] == 6))]
# print(filter.value_counts('batsman').head(1).index[0])
print(filter.groupby('batsman')['batsman'].count().sort_values(ascending=False).head(1).index[0]) #same as above line

print(ipl[(ipl['over'] > 15) & ((ipl['batsman_runs'] == 4) | (ipl['batsman_runs'] == 6))].groupby('batsman')['batsman'].count().sort_values(ascending=False).head(1).index[0])
...
print(vkData.groupby('bowling_team')['batsman_runs'].sum().sort_values(ascending=False).reset_index())
